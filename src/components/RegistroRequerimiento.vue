  ...
  <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
      <div class="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex justify-between items-center">
        <h2 class="text-2xl font-bold text-gray-800">{{ esEdicion ? 'Editar Requerimiento' : 'Nuevo Requerimiento' }}</h2>
        <button
          @click="$emit('cerrar')"
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const esEdicion = computed(() => !!props.requerimientoEditar)

    // Precargar datos cuando se recibe un requerimiento para editar
    watch(() => props.requerimientoEditar, (nuevoRequerimiento) => {
      if (nuevoRequerimiento) {
        ...
        // Resetear formulario cuando no hay requerimiento para editar
        formulario.value = {
          tipo: '',
          nombreCliente: '',
          numeroPoliza: '',
    ...
    const guardar = () => {
      ...
      if (esEdicion.value && props.requerimientoEditar) {
        datos.id = props.requerimientoEditar.id
      }
      emit('guardar', datos)
